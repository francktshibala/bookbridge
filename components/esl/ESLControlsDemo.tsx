...
interface ESLControlsDemoProps {
  onESLModeChange?: (enabled: boolean, level?: string) => void;
  variant?: 'floating' | 'inline';
}

export function ESLControlsDemo({ onESLModeChange, variant = 'floating' }: ESLControlsDemoProps) {
  const [eslEnabled, setESLEnabled] = useState(false);
  const [showLevelInfo, setShowLevelInfo] = useState(false);
  
  // Mock ESL user data
  const userESLLevel = 'B1';
  ...
  const toggleESLMode = () => {
    ...
    if (onESLModeChange) {
      onESLModeChange(newState, userESLLevel);
    }
  };
