...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'career-pivot-3' && bookId !== 'career-pivot-3-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Career Pivot #3 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Professional Transformation" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'career-pivot-3',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Career Pivot #3 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'career-pivot-3-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'career-pivot-3-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Career Pivot #3 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
