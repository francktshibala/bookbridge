...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Yellow Wallpaper for A1 level
    if (bookId !== 'gutenberg-1952-A1' && bookId !== 'yellow-wallpaper-a1' && bookId !== 'gutenberg-1952') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Yellow Wallpaper A1'
      }, { status: 400 });
    }

    console.log(`📄 Loading The Yellow Wallpaper bundles for level: ${level}`);
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Yellow Wallpaper A1'
      }, { status: 404 });
    }
    ...
    let chapters = null;
    try {
      const chaptersPath = path.join(process.cwd(), 'cache', 'gutenberg-1952-A1-chapters.json');
    ...
    return NextResponse.json({
      ...
      book: {
        id: bookId,
        title: bookContent?.title || 'The Yellow Wallpaper',
        author: bookContent?.author || 'Charlotte Perkins Gilman'
      },
      level: 'A1',
    ...
    console.error('The Yellow Wallpaper A1 API error:', error);
    return NextResponse.json({
      success: false,
