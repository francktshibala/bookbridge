...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'romantic-love-1' && bookId !== 'romantic-love-1-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Romantic Love #1 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Cross-Cultural Love Story" bundles for level: ${level}`);
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Romantic Love #1 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'romantic-love-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'romantic-love-1-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Romantic Love #1 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
