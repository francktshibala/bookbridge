...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "The Danger of a Single Story" TED Talk for A2 level
    if (bookId !== 'danger-of-single-story' && bookId !== 'danger-of-single-story-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Danger of a Single Story A2'
      }, {
        status: 400,
        headers: {
    ...
    if (chunkCount === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Danger of a Single Story A2'
      }, {
        status: 404,
    ...
    // Load preview text
    const previewTextPath = path.join(cacheDir, 'danger-of-single-story-A2-preview.txt');
    ...
    const previewAudioPath = path.join(cacheDir, 'danger-of-single-story-A2-preview-audio.json');
