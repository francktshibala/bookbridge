...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'medical-crisis-2' && bookId !== 'medical-crisis-2-a2') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Medical Crisis #2 Story A2' },
        { status: 400 }
      );
    }
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Medical Crisis #2 Story A2' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'medical-crisis-2-A2-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'medical-crisis-2-A2-preview-combined-audio.json');
