...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'refugee-journey-2' && bookId !== 'refugee-journey-2-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Second Chance Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Second Chance Story" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'refugee-journey-2',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Second Chance Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'refugee-journey-2-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'refugee-journey-2-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Second Chance Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
