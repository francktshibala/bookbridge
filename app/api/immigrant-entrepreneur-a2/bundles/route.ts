...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "Immigrant Entrepreneur: From Failure to Success" for A2 level
    if (bookId !== 'immigrant-entrepreneur' && bookId !== 'immigrant-entrepreneur-a2') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports Immigrant Entrepreneur: From Failure to Success A2'
      }, {
        status: 400,
        headers: {
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for Immigrant Entrepreneur: From Failure to Success A2'
      }, {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'immigrant-entrepreneur-A2-preview-combined.txt');
    ...
    const previewCombinedAudioPath = path.join(cacheDir, 'immigrant-entrepreneur-A2-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Immigrant Entrepreneur bundles:', error);
    return NextResponse.json({
      success: false,
