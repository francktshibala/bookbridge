...
export async function GET(request: NextRequest) {
  try {
    ...
    bookChunks.forEach((chunk, index) => {
      ...
      // Calculate dynamic timings using Sarah voice formula (0.30s per word for A1)
      let cumulativeTime = 0;
      const sentencesWithTimings = chunkSentences.map((text, sentenceIdx) => {
        const words = text.trim().split(/\s+/).length;
        const secondsPerWord = 0.30; // Sarah voice timing for A1
