...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'grief-to-purpose-1' && bookId !== 'grief-to-purpose-1-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Grief to Purpose #1 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Grief to Purpose: Finding Meaning in Loss" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'grief-to-purpose-1',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Grief to Purpose #1 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'grief-to-purpose-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'grief-to-purpose-1-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Grief to Purpose #1 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
