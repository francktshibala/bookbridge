...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'single-parent-rising-1' && bookId !== 'single-parent-rising-1-a2') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Single Parent Rising #1 Story A2' },
        { status: 400 }
      );
    }
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Single Parent Rising #1 Story A2' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'single-parent-rising-1-A2-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'single-parent-rising-1-A2-preview-combined-audio.json');
