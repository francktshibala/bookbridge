...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Gift of the Magi B1'
      }, { status: 404 });
    }
    ...
    try {
      const previewCachePath = path.join(process.cwd(), 'cache', 'gift-of-the-magi-B1-preview.txt');
      ...
      const previewAudioPath = path.join(process.cwd(), 'cache', 'gift-of-the-magi-B1-preview-audio.json');
      if (fs.existsSync(previewAudioPath)) {
        ...
        const previewAudioFileName = 'gift-of-the-magi/B1/preview.mp3';
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            let duration = 0;
            try {
              const cachePath = path.join(process.cwd(), 'cache', 'gift-of-the-magi-B1-preview-audio.json');
