...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'medical-crisis-1' && bookId !== 'medical-crisis-1-a1') {
    ...
    console.log(`🎤 Loading "Finding New Wings: A Medical Crisis Story" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'medical-crisis-1',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'medical-crisis-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'medical-crisis-1-A1-preview-combined-audio.json');
