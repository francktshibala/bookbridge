...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Great Gatsby for A2 level
    if (bookId !== 'great-gatsby-a2' && bookId !== 'great-gatsby') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Great Gatsby A2'
      }, { status: 400 });
    }

    console.log(`📗 Loading The Great Gatsby bundles for level: ${level}`);
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Great Gatsby A2'
      }, { status: 404 });
    }
    ...
    let chapters = null;
    try {
      const chaptersPath = path.join(process.cwd(), 'cache', 'great-gatsby-a2-chapters.json');
    ...
    return NextResponse.json({
      ...
      book: {
        id: bookId,
        title: bookContent?.title || 'The Great Gatsby',
        author: bookContent?.author || 'F. Scott Fitzgerald'
      },
      level: 'A2',
    ...
    console.error('The Great Gatsby A2 API error:', error);
    return NextResponse.json({
      success: false,
