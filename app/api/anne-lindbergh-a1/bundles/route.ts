...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "Anne Lindbergh" biography for A1 level
    if (bookId !== 'anne-lindbergh' && bookId !== 'anne-lindbergh-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports Anne Lindbergh A1'
      }, {
        status: 400,
        headers: {
    ...
    if (chunkCount === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for Anne Lindbergh A1'
      }, {
        status: 404,
    ...
    // Load preview text
    const previewTextPath = path.join(cacheDir, 'anne-lindbergh-A1-preview.txt');
    ...
    const previewAudioPath = path.join(cacheDir, 'anne-lindbergh-A1-preview-audio.json');
    ...
    console.error('❌ Error loading Anne Lindbergh bundles:', error);
