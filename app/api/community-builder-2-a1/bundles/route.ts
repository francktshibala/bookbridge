...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'community-builder-2' && bookId !== 'community-builder-2-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Community Builder #2 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Community Builder #2 Story" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'community-builder-2',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Community Builder #2 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'community-builder-2-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'community-builder-2-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Community Builder #2 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
