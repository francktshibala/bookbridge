...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'career-pivot-2' && bookId !== 'career-pivot-2-a1') {
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'career-pivot-2-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'career-pivot-2-A1-preview-combined-audio.json');
