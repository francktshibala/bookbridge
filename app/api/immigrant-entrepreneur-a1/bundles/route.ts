...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "Immigrant Entrepreneur: From Failure to Success" for A1 level
    if (bookId !== 'immigrant-entrepreneur' && bookId !== 'immigrant-entrepreneur-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports Immigrant Entrepreneur: From Failure to Success A1'
      }, {
        status: 400,
        headers: {
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'immigrant-entrepreneur-A1-preview-combined.txt');
    ...
    const previewCombinedAudioPath = path.join(cacheDir, 'immigrant-entrepreneur-A1-preview-combined-audio.json');
