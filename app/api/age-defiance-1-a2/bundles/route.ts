...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'age-defiance-1' && bookId !== 'age-defiance-1-a2') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Age Defiance #1 Story A2' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Age Defiance: Starting Over Later in Life" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'age-defiance-1',
        cefrLevel: 'A2'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Age Defiance #1 Story A2' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'age-defiance-1-A2-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'age-defiance-1-A2-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Age Defiance #1 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
