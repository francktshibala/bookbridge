...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'cultural-bridge-1' && bookId !== 'cultural-bridge-1-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Medical Crisis #1 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Between Two Worlds: A Cultural Bridge Story" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'cultural-bridge-1',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Medical Crisis #1 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'cultural-bridge-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'cultural-bridge-1-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Medical Crisis #1 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
