...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Tell-Tale Heart for A1 level
    if (bookId !== 'tell-tale-heart' && bookId !== 'tell-tale-heart-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Tell-Tale Heart A1'
      }, { status: 400 });
    }
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Tell-Tale Heart A1'
      }, { status: 404 });
    }
    ...
    try {
      const previewCachePath = path.join(process.cwd(), 'cache', 'tell-tale-heart-A1-preview.txt');
      ...
      const previewAudioPath = path.join(process.cwd(), 'cache', 'tell-tale-heart-A1-preview-audio.json');
      if (fs.existsSync(previewAudioPath)) {
        ...
        const previewAudioFileName = 'tell-tale-heart/A1/preview.mp3';
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            let duration = 0;
            try {
              const cachePath = path.join(process.cwd(), 'cache', 'tell-tale-heart-A1-preview-audio.json');
    ...
    console.error('The Tell-Tale Heart A1 API error:', error);
    return NextResponse.json({
      success: false,
