...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Tell-Tale Heart for B1 level
    if (bookId !== 'tell-tale-heart' && bookId !== 'tell-tale-heart-b1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Tell-Tale Heart B1'
      }, { status: 400 });
    }
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Tell-Tale Heart B1'
      }, { status: 404 });
    }
    ...
    try {
      const previewCachePath = path.join(process.cwd(), 'cache', 'tell-tale-heart-B1-preview.txt');
      ...
      const previewAudioPath = path.join(process.cwd(), 'cache', 'tell-tale-heart-B1-preview-audio.json');
      if (fs.existsSync(previewAudioPath)) {
        ...
        const previewAudioFileName = 'tell-tale-heart/B1/preview.mp3';
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            let duration = 0;
            try {
              const cachePath = path.join(process.cwd(), 'cache', 'tell-tale-heart-B1-preview-audio.json');
    ...
    console.error('The Tell-Tale Heart B1 API error:', error);
    return NextResponse.json({
      success: false,
