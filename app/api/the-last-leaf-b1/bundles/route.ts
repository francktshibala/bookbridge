...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Last Leaf for B1 level
    if (bookId !== 'the-last-leaf' && bookId !== 'the-last-leaf-b1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Last Leaf B1'
      }, { status: 400 });
    }

    console.log(`💎 Loading The Last Leaf bundles for level: ${level}`);
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Last Leaf B1'
      }, { status: 404 });
    }
    ...
    try {
      const previewCachePath = path.join(process.cwd(), 'cache', 'the-last-leaf-B1-preview.txt');
      ...
      const previewAudioPath = path.join(process.cwd(), 'cache', 'the-last-leaf-B1-preview-audio.json');
      if (fs.existsSync(previewAudioPath)) {
        ...
        const previewAudioFileName = 'the-last-leaf/B1/preview.mp3';
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            let duration = 0;
            try {
              const cachePath = path.join(process.cwd(), 'cache', 'the-last-leaf-B1-preview-audio.json');
    ...
    return NextResponse.json({
      ...
      book: {
        id: 'the-last-leaf',
        title: bookContent?.title || 'The Last Leaf',
    ...
    console.error('The Last Leaf B1 API error:', error);
    return NextResponse.json({
      success: false,
