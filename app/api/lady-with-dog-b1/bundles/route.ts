...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Lady with the Dog for B1 level
    if (bookId !== 'lady-with-dog' && bookId !== 'lady-with-dog-b1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Lady with the Dog B1'
      }, {
        status: 400,
        headers: {
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Lady with the Dog B1'
      }, {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    try {
      const previewCachePath = path.join(process.cwd(), 'cache', 'lady-with-dog-B1-preview.txt');
      ...
      const previewAudioPath = path.join(process.cwd(), 'cache', 'lady-with-dog-B1-preview-audio.json');
      if (fs.existsSync(previewAudioPath)) {
        ...
        const previewAudioFileName = 'lady-with-dog/B1/preview.mp3';
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            let duration = 0;
            try {
              const cachePath = path.join(process.cwd(), 'cache', 'lady-with-dog-B1-preview-audio.json');
    ...
    console.error('The Lady with the Dog B1 API error:', error);
    return NextResponse.json({
      success: false,
