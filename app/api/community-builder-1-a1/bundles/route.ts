...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "Community Builder Story" for A1 level
    if (bookId !== 'community-builder-1' && bookId !== 'community-builder-1-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports Community Builder Story A1'
      }, {
        status: 400,
        headers: {
    ...
    console.log(`🎤 Loading "Community Builder Story" bundles for level: ${level}`);

    // Get bundles from BookChunk table with audio duration metadata (FAST PATH - no extra count query)
    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'community-builder-1',
        cefrLevel: 'A1'
      },
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for Community Builder Story A1'
      }, {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'community-builder-1-A1-preview-combined.txt');
    ...
    const previewCombinedAudioPath = path.join(cacheDir, 'community-builder-1-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Community Builder Story bundles:', error);
    return NextResponse.json({
      success: false,
