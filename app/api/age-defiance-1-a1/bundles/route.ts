...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'age-defiance-1' && bookId !== 'age-defiance-1-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Age Defiance #1 Story A1' },
        { status: 400 }
      );
    }
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Age Defiance #1 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'age-defiance-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'age-defiance-1-A1-preview-combined-audio.json');
