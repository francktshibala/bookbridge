...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "First-Gen Student Teaching Dad to Read" for A2 level
    if (bookId !== 'teaching-dad-to-read' && bookId !== 'teaching-dad-to-read-a2') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports First-Gen Student Teaching Dad to Read A2'
      }, {
        status: 400,
        headers: {
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for First-Gen Student Teaching Dad to Read A2'
      }, {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'teaching-dad-to-read-A2-preview-combined.txt');
    ...
    const previewCombinedAudioPath = path.join(cacheDir, 'teaching-dad-to-read-A2-preview-combined-audio.json');
