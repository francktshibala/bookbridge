...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Legend of Sleepy Hollow for A1 level
    if (bookId !== 'sleepy-hollow-enhanced' && bookId !== 'sleepy-hollow-a1' && bookId !== 'sleepy-hollow') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Legend of Sleepy Hollow A1'
      }, { status: 400 });
    }

    console.log(`🎃 Loading The Legend of Sleepy Hollow bundles for level: ${level}`);
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Legend of Sleepy Hollow A1'
      }, { status: 404 });
    }
    ...
    let chapters = null;
    try {
      const chaptersPath = path.join(process.cwd(), 'cache', 'sleepy-hollow-enhanced-chapters.json');
    ...
    return NextResponse.json({
      ...
      book: {
        id: bookId,
        title: bookContent?.title || 'The Legend of Sleepy Hollow',
        author: bookContent?.author || 'Washington Irving'
      },
      level: 'A1',
    ...
    console.error('The Legend of Sleepy Hollow A1 API error:', error);
    return NextResponse.json({
      success: false,
