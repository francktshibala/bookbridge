...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'second-chance-1' && bookId !== 'second-chance-1-a1') {
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'second-chance-1-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'second-chance-1-A1-preview-combined-audio.json');
