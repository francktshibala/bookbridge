...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports The Dead for A2 level
    if (bookId !== 'the-dead' && bookId !== 'the-dead-a2') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Dead A2'
      }, {
        status: 400,
        headers: {
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Dead A2'
      }, {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('The Dead A2 API error:', error);
    return NextResponse.json({
      success: false,
