...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookId !== 'refugee-journey-3' && bookId !== 'refugee-journey-3-a1') {
      return NextResponse.json(
        { success: false, error: 'This API only supports Refugee Journey #3 Story A1' },
        { status: 400 }
      );
    }

    console.log(`🎤 Loading "Refugee Family Journey" bundles for level: ${level}`);

    const bookChunks = await prisma.bookChunk.findMany({
      where: {
        bookId: 'refugee-journey-3',
        cefrLevel: 'A1'
      },
      orderBy: {
        chunkIndex: 'asc'
    ...
    if (!bookChunks || bookChunks.length === 0) {
      return NextResponse.json(
        { success: false, error: 'No bundles found for Refugee Journey #3 Story A1' },
        { status: 404 }
      );
    }
    ...
    const previewCombinedTextPath = path.join(cacheDir, 'refugee-journey-3-A1-preview-combined.txt');
    const previewCombined = fs.existsSync(previewCombinedTextPath)
      ? fs.readFileSync(previewCombinedTextPath, 'utf8').trim()
      : null;

    const previewCombinedAudioPath = path.join(cacheDir, 'refugee-journey-3-A1-preview-combined-audio.json');
    ...
    console.error('❌ Error loading Refugee Journey #3 Story bundles:', error);
    return NextResponse.json(
      {
        success: false,
