...
export async function GET(request: NextRequest) {
  try {
    ...
    // This API supports "The Danger of a Single Story" TED Talk for A1 level
    if (bookId !== 'danger-of-single-story' && bookId !== 'danger-of-single-story-a1') {
      return NextResponse.json({
        success: false,
        error: 'This API only supports The Danger of a Single Story A1'
      }, {
        status: 400,
        headers: {
    ...
    console.log(`🎤 Loading "The Danger of a Single Story" bundles for level: ${level}`);

    // Fast-fail check: count bundles first to avoid timeout
    const chunkCount = await prisma.bookChunk.count({
      where: {
        bookId: 'danger-of-single-story',
        cefrLevel: 'A1'
      }
    });
    ...
    if (chunkCount === 0) {
      return NextResponse.json({
        success: false,
        error: 'No bundles found for The Danger of a Single Story A1'
      }, {
        status: 404,
    ...
    // Load preview text
    const previewTextPath = path.join(cacheDir, 'danger-of-single-story-A1-preview.txt');
    ...
    const previewAudioPath = path.join(cacheDir, 'danger-of-single-story-A1-preview-audio.json');
    ...
    console.error('❌ Error loading The Danger of a Single Story bundles:', error);
