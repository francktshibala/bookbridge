/**
 * Generate combined preview audio for Career Pivot #3
 * Uses Daniel voice (professional/business narrator)
 * Applies Enhanced Timing v3 for perfect sync
 */
...
// VALIDATED VOICE ID - Daniel (Professional/business narrator)
...
// PRODUCTION VOICE SETTINGS - Daniel (Career Pivot narration)
...
function calculateEnhancedTimingV3(sentences, totalDuration) {
  ...
  const timings = sentencePenalties.map((item, index) => {
    const charCount = item.sentence.length;
    const charProportion = charCount / totalCharacters;
    const baseDuration = remainingDuration * charProportion;
    const adjustedDuration = baseDuration + item.pausePenalty;
...
async function main() {
  console.log('🎤 Generating combined preview audio for "Career Pivot #3"');
  ...
  if (!fs.existsSync(combinedTextPath)) {
    throw new Error(`Combined text file not found: ${combinedTextPath}. Run generate-career-pivot-3-preview-combined.js first.`);
  }
