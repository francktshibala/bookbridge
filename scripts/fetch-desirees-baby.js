...
/**
 * Fetch "The Story of an Hour" by Kate Chopin from Project Gutenberg
...
const BOOK_INFO = {
  ...
  author: 'Kate Chopin',
  gutenbergUrl: 'https://www.gutenberg.org/files/160/160-0.txt', // Kate Chopin collection
  startMarker: "DÉSIRÉE'S BABY",
  endMarker: 'A RESPECTABLE WOMAN'
};

async function fetchDesireeseBaby() {
  console.log('👶 Fetching "Désirée\'s Baby" by Kate Chopin from Project Gutenberg...');
  ...
  try {
    ...
    // Fetch the full Kate Chopin collection
    ...
    // Extract "Désirée's Baby" story from the collection
    const extractedText = extractStory(fullText, BOOK_INFO.startMarker, BOOK_INFO.endMarker);

    if (!extractedText) {
      throw new Error('Could not extract "Désirée\'s Baby" from the collection');
    }
    ...
    // Save to cache
    const outputPath = path.join(cacheDir, 'desirees-baby-raw.txt');
    ...
    const metadataPath = path.join(cacheDir, 'desirees-baby-metadata.json');
    fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));
    console.log(`📋 Metadata saved to: ${metadataPath}`);

    console.log('✅ "Désirée\'s Baby" fetch completed successfully!');
    ...
    console.error('❌ Error fetching "Désirée\'s Baby":', error.message);
    process.exit(1);
  }
}
...
function extractStory(fullText, startMarker, endMarker) {
  // Find the start of "Désirée's Baby" story
  const startIndex = fullText.indexOf(startMarker);
  if (startIndex === -1) {
    throw new Error(`Could not find start marker: ${startMarker}`);
  }

  // Find the next story (end marker)
