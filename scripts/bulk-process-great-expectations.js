...
async function detectChunkCount() {
  ...
  const maxChunks = 1000 // Maximum to check (Great Expectations is long)
  let detectedCount = 0
...
async function main() {
  ...
  console.log(`${colors.bright}${colors.magenta}📚 BULK PROCESSING GREAT EXPECTATIONS${colors.reset}`)
  console.log('='.repeat(55))
  console.log(`  Book ID: ${BOOK_ID}`)
  console.log(`  Author: Charles Dickens`)
  ...
  const estimatedHours = Math.ceil(totalSimplifications / 100) // ~100 per hour estimate
  console.log(`  ${colors.yellow}⏰ Estimated processing time: ${estimatedHours} hours${colors.reset}`)
  console.log('')
  ...
  // Processing warning for large books
  if (missing.length > 1000) {
    console.log('')
    console.log(`${colors.yellow}⚠️  WARNING: This is a LARGE book with ${missing.length} simplifications to process!${colors.reset}`)
    console.log(`   Consider running this on multiple computers to speed up processing`)
    console.log(`   Expected time: ${estimatedHours} hours`)
  }
  ...
  for (let i = 0; i < missing.length; i += BATCH_SIZE) {
    ...
    const remainingMinutes = Math.ceil(remainingBatches * (DELAY_BETWEEN_BATCHES / 1000) / 60)
    if (remainingMinutes > 0) {
      console.log(`  ${colors.cyan}⏰ Estimated time remaining: ${remainingMinutes} minutes${colors.reset}`)
    }
  ...
  if (finalCheck.total >= chunkCount * CEFR_LEVELS.length) {
    console.log('')
    console.log(`${colors.green}${colors.bright}🎉 GREAT EXPECTATIONS FULLY PROCESSED!${colors.reset}`)
    console.log(`${colors.green}📚 Charles Dickens modernized for all CEFR levels!${colors.reset}`)
  }
