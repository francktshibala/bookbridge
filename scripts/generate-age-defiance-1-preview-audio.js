...
async function generateCombinedPreviewAudio(combinedText, storyId, level) {
  ...
  try {
    console.log(`   🗣️ Voice: ${SARAH_VOICE_ID} (Sarah)`);
...
async function main() {
  console.log('🎤 Generating combined preview audio for "Age Defiance #1"');
  ...
  if (!fs.existsSync(combinedTextPath)) {
    throw new Error(`Combined text file not found: ${combinedTextPath}. Run generate-age-defiance-1-preview-combined.js first.`);
  }
