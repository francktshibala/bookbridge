...
console.log(`📚 INTEGRATING "How Great Leaders Inspire Action" - B1 LEVEL INTO DATABASE`);
...
async function integrateDatabase() {
  ...
  const originalTextPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-original.txt');
  ...
  const simplifiedTextPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-b1-simplified.txt');
  ...
  const metadataPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-B1-bundles-metadata.json');
  ...
  console.log(`\n🔨 Step 5: Deleting existing B1 chunks (if any)...`);
  ...
  for (const bundle of metadata) {
    try {
      ...
      const sentenceTimings = bundle.sentences.map((sentence, idx) => ({
        text: sentence,
        start: idx * avgDurationPerSentence,
        end: (idx + 1) * avgDurationPerSentence,
      ...
      const relativeAudioPath = bundle.audioUrl.includes('/audio-files/')
        ...
        : `how-great-leaders-inspire-action/B1/bundle_${bundle.bundleIndex}.mp3`;
  ...
  console.log(`=`.repeat(60));
  console.log(`📚 Book: How Great Leaders Inspire Action (${CEFR_LEVEL})`);
