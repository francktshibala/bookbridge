...
if (!VALID_LEVELS.includes(targetLevel)) {
  ...
  console.log('Usage: node scripts/generate-immigrant-entrepreneur-preview-combined.js [A1|A2]');
  process.exit(1);
}
...
async function generateCombinedPreviewAudio(combinedText, storyId, level) {
  ...
  try {
    console.log(`   🗣️ Voice: ${DANIEL_VOICE_ID} (Daniel)`);
...
async function main() {
  console.log('🎤 Generating combined preview for "Immigrant Entrepreneur: From Failure to Success"');

  // Load combined text from cache
  const cacheDir = path.join(process.cwd(), 'cache');
  const previewTextPath = path.join(cacheDir, `${STORY_ID}-preview.txt`);
  const hookTextPath = path.join(cacheDir, `${STORY_ID}-hook.txt`);
  const backgroundTextPath = path.join(cacheDir, `${STORY_ID}-background.txt`);
  const combinedTextPath = path.join(cacheDir, `${STORY_ID}-${CEFR_LEVEL}-preview-combined.txt`);

  // Check if preview.txt exists, otherwise create a preview from the story
  let previewContent;
  if (fs.existsSync(previewTextPath)) {
    previewContent = fs.readFileSync(previewTextPath, 'utf8').trim();
  } else {
    // Generate preview text (marketing copy, 50-75 words)
    previewContent = 'A powerful story about immigrant entrepreneurs who lost everything—to fire, to war, to financial struggles—but refused to give up. Through hard work, community support, and unwavering determination, they rebuilt their dreams and found success. This inspiring tale shows that sometimes, losing everything is the beginning of something even greater.';
  }

  const hookContent = fs.readFileSync(hookTextPath, 'utf8').trim();
  const backgroundContent = fs.readFileSync(backgroundTextPath, 'utf8').trim();

  // Combine with "About This Story" header and double newlines
  const combinedText = `About This Story\n\n${previewContent}\n\n${hookContent}\n\n${backgroundContent}`;
  fs.writeFileSync(combinedTextPath, combinedText, 'utf8');
