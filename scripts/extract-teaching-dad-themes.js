/**
 * Extract themes from multiple sources for "First-Gen Student Teaching Dad to Read"
 * 
 * IMPORTANT: This script processes manually fetched source material.
 * Before running:
 * 1. Manually fetch source articles and save to cache/teaching-dad-to-read-source-*.txt
...
async function extractThemes() {
  ...
  // Read source files
  const sourceFiles = [
    path.join(CACHE_DIR, `${STORY_ID}-source-1.txt`),
    path.join(CACHE_DIR, `${STORY_ID}-source-2.txt`),
    path.join(CACHE_DIR, `${STORY_ID}-source-3.txt`),
  ];
  ...
  if (sources.length === 0) {
    ...
    console.log('2. Save raw text to cache/teaching-dad-to-read-source-*.txt');
  ...
  const prompt = `You are extracting THEMES and EMOTIONAL MOMENTS from source material about children teaching their parents to read or learn English.
DO NOT copy any text. Extract only:
1. Themes (intergenerational learning, role reversal, family dynamics, education, empowerment, patience, shame, breakthrough)
2. Emotional moments (7 moments: parent's shame/embarrassment, child recognizing need, first teaching attempt, hesitation/fear, breakthrough moment, growing confidence, relationship transformation)
