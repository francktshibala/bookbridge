...
const CEFR_LEVEL = process.argv[2] || 'A1'; // Pass level as argument
const TEST_LIMIT = process.env.TEST_LIMIT ? parseInt(process.env.TEST_LIMIT) : null;
const CACHE_FILE = `./cache/yellow-wallpaper-${CEFR_LEVEL}-simplified.json`;

async function simplifyYellowWallpaper() {
  console.log(`📚 Starting Yellow Wallpaper simplification to ${CEFR_LEVEL} level...`);
  if (TEST_LIMIT) {
