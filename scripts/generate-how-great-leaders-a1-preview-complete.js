...
// Daniel voice settings (November 2025 production standard)
const DANIEL_VOICE_SETTINGS = {
  voice_id: 'onwK4e9ZLuTAKqWW03F9',  // Daniel - Natural, warm voice
  voice_name: 'Daniel',
...
console.log(`🗣️ Voice: ${DANIEL_VOICE_SETTINGS.voice_name} (${DANIEL_VOICE_SETTINGS.voice_id})`);
...
const previewText = `In this powerful TED Talk, leadership expert Simon Sinek explains the Golden Circle: Why, How, and What. Through examples like Apple, the Wright Brothers, and Martin Luther King Jr., he shows why great leaders start with "why" instead of "what." A transformative message about purpose, inspiration, and what makes some leaders truly inspire action.`;
...
async function generatePreviewAudio() {
  ...
  console.log(`   Voice: ${DANIEL_VOICE_SETTINGS.voice_name}`);
  console.log(`   Text length: ${previewText.length} characters`);

  try {
    // Call ElevenLabs API
    const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${DANIEL_VOICE_SETTINGS.voice_id}`, {
      method: 'POST',
      headers: {
        'Accept': 'audio/mpeg',
    ...
    const metadata = {
      ...
      audio: {
        url: publicUrl,
        duration: duration,
        voice: DANIEL_VOICE_SETTINGS.voice_name,
