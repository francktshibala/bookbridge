...
// Preview text for Helen Keller memoir (A1 level - simple, short sentences)
// Format: Marketing copy that DESCRIBES the story, not excerpts from it
// Required elements: Content type, Person/subject, Main theme, Key insight, Impact
const PREVIEW_TEXT = "In this inspiring story, Helen Keller was a young girl who could not see or hear. She felt very frustrated and alone. Then a teacher named Anne Sullivan came to help her. Through patience and love, Anne taught Helen that words have meaning. This is a powerful message about hope, education, and never giving up.";
...
async function main() {
  console.log('🎤 Generating preview for Helen Keller\'s "The Story of My Life"');

  // Validate preview text length
  const wordCount = PREVIEW_TEXT.split(/\s+/).filter(word => word.length > 0).length;
  console.log(`\n📝 Preview text (${wordCount} words):`);
  console.log(`   "${PREVIEW_TEXT}"`);

  if (wordCount < 50 || wordCount > 75) {
    console.warn(`⚠️ Warning: Preview text is ${wordCount} words (target: 50-75 words)`);
  }
  ...
  fs.writeFileSync(previewTextPath, PREVIEW_TEXT);
  console.log(`\n✅ Saved preview text to cache`);

  // Generate audio
  const audioMetadata = await generatePreviewAudio(PREVIEW_TEXT, BOOK_ID, CEFR_LEVEL);

  // Save audio metadata to cache
  const audioMetadataPath = path.join(cacheDir, `${BOOK_ID}-${CEFR_LEVEL}-preview-audio.json`);
  const metadata = {
    bookId: BOOK_ID,
    level: CEFR_LEVEL,
    audio: audioMetadata,
