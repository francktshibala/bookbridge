...
// VOICE MAPPING FOR TELL-TALE HEART: A1 → Daniel
...
if (!targetLevel) {
  ...
  console.log('Usage: node scripts/generate-tell-tale-heart-bundles.js [A1] [--pilot]');
  process.exit(1);
}
...
const voiceName = CEFR_LEVEL === 'A1' ? 'Daniel' : 'Daniel';
...
async function checkElevenLabsQuota() {
  try {
    ...
    // Estimate cost (rough: ~2000 words = ~10,000 chars = ~$1.65)
    const estimatedCost = Math.ceil(2000 / 1000) * 16.5; // cents
    console.log(`   Estimated cost for ~2000 words: ~$${(estimatedCost / 100).toFixed(2)}`);
...
async function generateElevenLabsAudio(text, voiceSettings, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // Validate voice ID
      if (!VALIDATED_VOICES.daniel || voiceSettings.voice_id !== VALIDATED_VOICES.daniel) {
...
async function generateTellTaleHeartBundles() {
  console.log(`\n📚 Starting bundle generation for "The Tell-Tale Heart" (${CEFR_LEVEL})...`);
  ...
  try {
    ...
    // For A1, delete existing bundles to ensure clean override with Daniel voice
    if (CEFR_LEVEL === 'A1') {
      console.log('🗑️ Deleting existing A1 bundles for clean regeneration with Daniel voice...');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generateTellTaleHeartBundles()
    .then(() => {
      console.log('\n🎉 The Tell-Tale Heart bundle generation completed!');
      process.exit(0);
    })
    .catch((error) => {
...
export { generateTellTaleHeartBundles };
