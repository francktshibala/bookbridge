...
async function writeMainStory() {
  ...
  if (!fs.existsSync(themesFile)) {
    console.error('❌ Themes file not found. Run extract-teen-translating-themes.js first.');
    return;
  }
...
- Structure: struggle → crisis → breakthrough → confidence
...
1. Starts with the emotional hook energy (medical emergency scene)
2. Develops the 7 emotional moments in sequence
3. Shows the emotional arc: struggle → crisis → breakthrough → confidence
4. Emphasizes ESL resonance multipliers throughout
5. Ends with realization of strength and growth
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a skilled storyteller writing original narratives for ESL learners. You NEVER copy text from sources - you create original stories based on themes and emotional moments. Your writing is clear, engaging, and appropriate for A2 level learners.',
        },
        {
          role: 'user',
          content: prompt,
