...
// VALIDATED VOICE ID - Jane (Emotional/personal growth narrator)
...
// PRODUCTION VOICE SETTINGS - Jane (Workplace Discrimination narration)
...
async function generateBundles() {
  try {
    const storyFilePath = path.join(process.cwd(), 'cache', `${STORY_ID}-A1-simplified.txt`);
    ...
    const SENTENCES_PER_BUNDLE = 5; // ~5 sentences per bundle
    const bundles = [];
    ...
    for (let i = 0; i < bundlesToGenerate; i++) {
      ...
      console.log(`   🎤 Generating audio with ElevenLabs (Jane voice)...`);
      const audioBuffer = await generateElevenLabsAudio(bundleText, JANE_VOICE_SETTINGS);
