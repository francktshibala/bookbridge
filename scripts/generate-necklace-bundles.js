...
const SARAH_VOICE_SETTINGS = {
  ...
  voice_settings: {
    ...
    similarity_boost: 0.8,             // Enhanced presence (+0.05 from defaults)
...
// Check ElevenLabs API quota and rate limits (MASTER_MISTAKES_PREVENTION.md requirement)
async function checkElevenLabsQuota() {
  try {
    ...
    // Estimate cost for The Necklace A1 (283 sentences × ~30 words × ~5 chars = ~42k chars)
    const estimatedChars = 283 * 30 * 5; // Rough estimate
...
async function generateNecklaceBundles() {
  ...
  try {
    ...
    if (CEFR_LEVEL === 'A1' || CEFR_LEVEL === 'A2') {
      ...
      console.log(`   ✅ Deleted ${deletedCount.count} existing ${CEFR_LEVEL} bundles`);
    }

    // Check for existing bundles to enable resume (for other levels)
    ...
    for (const bundle of bundlesToGenerate) {
      try {
        ...
        // Flexible bundle size validation (final bundle can be 3-4 sentences)
