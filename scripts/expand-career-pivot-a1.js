/**
 * Expand Career Pivot story to meet target length (1,600-1,700 words, ~20 minutes)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 Career Pivot story to meet target length...');
  
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-career-pivot-story.js first.');
    return;
  }
...
- Use generic character names (Maria, David, Sofia) - NOT real names
...
Expand the story by:
1. Adding more detail to existing scenes (what did she feel? what did she see? what did she think?)
2. Adding more scenes showing the burnout, work obsession, and family disconnection
3. Expanding the wake-up call moment with more detail
4. Adding more scenes showing deliberation and fear before quitting
5. Adding more scenes showing the courage moment (walking around office, submitting resignation)
6. Adding more scenes showing transformation and family reconnection
7. Including more emotional moments and internal thoughts
8. Adding more dialogue between characters
9. Showing more of the work-life imbalance and its impact
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a skilled storyteller expanding narratives for A1 level ESL learners. You maintain simple, clear language appropriate for A1 level (short sentences, simple words, basic grammar). You always use generic character names, never real names from sources.',
        },
        {
          role: 'user',
          content: prompt,
