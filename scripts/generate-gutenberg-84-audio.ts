  ...
  for (const [level, items] of Object.entries(byLevel)) {
    ...
    for (const item of items) {
      try {
        ...
        // Step 1: Generate audio using TTS API
        ...
        // Store audio URL in a simple audio_files table 
        // (Will create the record manually since schema is complex)
        try {
          await prisma.$executeRaw`
            INSERT INTO audio_files (book_id, cefr_level, chunk_index, audio_url, provider, voice_id, file_size, created_at)
            VALUES (${'gutenberg-84'}, ${item.targetLevel}, ${item.chunkIndex}, ${publicUrl}, 'openai', 'nova', ${audioBuffer.length}, NOW())
