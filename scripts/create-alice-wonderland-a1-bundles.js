...
const INPUT_FILE = path.join(CACHE_DIR, 'alice-wonderland-modernized.json');
const OUTPUT_FILE = path.join(CACHE_DIR, 'alice-wonderland-a1-bundles.json');

const BOOK_ID = 'gutenberg-11';
const SIMPLIFIED_BOOK_ID = 'gutenberg-11-A1';
const CEFR_LEVEL = 'A1';
...
async function createAliceWonderlandA1Bundles() {
  try {
    console.log(`📚 Creating Alice in Wonderland A1 bundles...\\n`);
    ...
    if (!fs.existsSync(INPUT_FILE)) {
      throw new Error('Modernized text not found. Run modernize-alice-wonderland.js first.');
    }
    ...
    const batchSize = 20; // Proven optimal size from Romeo & Juliet
    ...
    for (let i = 0; i < sentences.length; i += batchSize) {
      ...
      // CRITICAL: Verify sentence count matches (lesson from Romeo & Juliet)
      if (simplifiedBatch.length !== batch.length) {
        ...
        if (retryBatch.length === batch.length) {
          ...
          // Final fallback: adjust to exact count
          ...
          while (adjustedBatch.length < batch.length) {
            adjustedBatch.push(`Alice had fun.`); // Pad if too few
          }
    ...
    const bundleData = {
      ...
      title: "Alice's Adventures in Wonderland (A1 Level)",
    ...
    console.log(`\\n🎉 Alice in Wonderland A1 bundles created successfully!`);
...
4. Convert Alice in Wonderland story to simple language
5. Keep whimsical character names (Alice, White Rabbit, Cheshire Cat)
...
async function simplifyBatchToA1(sentences, startIndex, totalSentences) {
  const prompt = `SIMPLIFY these Alice in Wonderland sentences to A1 CEFR level.
...
ALICE IN WONDERLAND TO A1 VOCABULARY:
- "wonderland" → "magic place"
...
- Keep character names: Alice, White Rabbit, Queen, Mad Hatter, etc.
- Simplify fantasy elements but keep magical feel
- Use present tense when possible
- Remove complex Victorian phrases
- Keep the wonder and adventure spirit
...
["Alice saw a white rabbit.", "The rabbit ran very fast.", "Alice followed the rabbit."]`;
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        result = sentences.map((_, i) => `Alice had an adventure.`);
      }
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createAliceWonderlandA1Bundles().catch(console.error);
}

export { createAliceWonderlandA1Bundles };