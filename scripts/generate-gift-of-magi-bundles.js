...
if (!targetLevel) {
  ...
  console.log('Usage: node scripts/generate-gift-of-magi-bundles.js [A1|A2|B1] [--pilot]');
  process.exit(1);
}
...
async function checkElevenLabsQuota() {
  try {
    ...
    // Estimate cost for Gift of the Magi A1 (small book, ~51 sentences)
    const estimatedChars = 51 * 15 * 5; // Rough estimate
...
async function generateGiftOfMagiBundles() {
  console.log(`\n📚 Starting bundle generation for "The Gift of the Magi" (${CEFR_LEVEL})...`);
  ...
  try {
    ...
    // Load simplified text from cache (TXT format for gift-of-magi)
    ...
    if (!fs.existsSync(simplifiedFilePath)) {
      throw new Error(`Simplified cache not found: ${simplifiedFilePath}. Run simplify script first.`);
    }

    const simplifiedText = fs.readFileSync(simplifiedFilePath, 'utf8').trim();
    ...
    console.log(`📖 Loaded ${cleanSentences.length} simplified sentences`);

    // Create bundles (4 sentences each)
    const BUNDLE_SIZE = 4;
    const bundles = [];

    for (let i = 0; i < cleanSentences.length; i += BUNDLE_SIZE) {
      const bundleSentences = cleanSentences.slice(i, i + BUNDLE_SIZE);
