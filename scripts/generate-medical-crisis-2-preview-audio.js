...
// VALIDATED VOICE ID - Jane (Family/emotional stories)
...
// PRODUCTION VOICE SETTINGS - Jane (Medical Crisis narration)
...
if (!VALID_LEVELS.includes(targetLevel)) {
  ...
  console.log('Usage: node scripts/generate-medical-crisis-2-preview-audio.js [A1|A2]');
  process.exit(1);
}
...
async function main() {
  console.log('🎤 Generating combined preview audio for "Medical Crisis #2"');
  ...
  if (!fs.existsSync(combinedTextPath)) {
    throw new Error(`Combined text file not found: ${combinedTextPath}. Run generate-medical-crisis-2-preview-combined.js first.`);
  }
