/**
 * Expand the A1 Second Chance story to meet target length (15-20 minutes A1 level)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 Second Chance story to meet target length...');

  // Load current story
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-second-chance-story.js first.');
    return;
  }
  ...
  const prompt = `You are expanding a Second Chance redemption story for ESL learners at A1 level. The current story is ${currentWordCount} words, and you need to expand it to approximately ${targetWordCount} words (target: 20 minutes reading time).
...
- Keep generic character names (Maria, David, Sofia, etc.)
...
- Add more detail to emotional moments (especially high-weight moments):
  * Despair/rock bottom (facing death in prison, "life is over", addiction crisis)
  ...
  * Breakthrough moments (first time allowed to think/build, vulnerability breakthrough, spiritual awakening)
...
Expand the story by adding detail, emotion, dialogue, and scenes while keeping all existing content. Maintain A1 level language (short sentences, simple words). Return ONLY the expanded story text, no explanations.`;
  ...
  try {
    ...
    // Clean markdown/metadata characters (Step 7.25)
