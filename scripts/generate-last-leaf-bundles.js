...
// VOICE MAPPING FOR LAST LEAF: B1 → Jane
...
if (!targetLevel) {
  console.error('❌ Error: Please specify a CEFR level (B1)');
  console.log('Usage: node scripts/generate-last-leaf-bundles.js [B1] [--pilot]');
  process.exit(1);
}
...
async function generateLastLeafBundles() {
  console.log(`\n📚 Starting bundle generation for "The Last Leaf" (${CEFR_LEVEL})...`);
  ...
  try {
    ...
    // For B1, delete existing bundles to ensure clean override
    if (CEFR_LEVEL === 'B1') {
      console.log(`🗑️ Deleting existing ${CEFR_LEVEL} bundles for clean regeneration with Jane voice...`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generateLastLeafBundles()
    .then(() => {
      console.log('\n🎉 The Last Leaf bundle generation completed!');
      process.exit(0);
    })
    .catch((error) => {
...
export { generateLastLeafBundles };
