...
const GUTENBERG_ID = '902';
const GUTENBERG_URL = `https://www.gutenberg.org/files/${GUTENBERG_ID}/${GUTENBERG_ID}-0.txt`;

async function fetchTheDevotedFriend() {
  console.log('📖 Fetching "The Devoted Friend" by Oscar Wilde...');
  console.log(`🔗 Source: Project Gutenberg ID ${GUTENBERG_ID} (The Happy Prince and Other Tales)`);
  ...
  try {
    ...
    // Extract "The Devoted Friend" story from the collection
    const startMarker = 'ONE morning the old Water-rat put his head out of his hole';
    const endMarker = 'The Remarkable Rocket'; // Next story title
    ...
    const cleanedText = extractedText
      ...
      .replace(/The Devoted Friend\s*/i, '') // Remove title if present
      .trim();
    ...
    console.error('❌ Failed to fetch The Devoted Friend:', error.message);
    throw error;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  fetchTheDevotedFriend()
    .then(() => console.log('\n✅ The Devoted Friend fetched successfully!'))
    .catch(console.error);
}

export { fetchTheDevotedFriend };