...
- Keep proper nouns: Jordan, Marcus, Sarah, etc.
`;
...
6. Keep proper nouns unchanged (Jordan, Marcus, Sarah, etc.)
...
async function simplifyStory() {
  const inputFile = path.join(process.cwd(), 'cache', 'workplace-discrimination-1-original.txt');
  const outputFile = path.join(process.cwd(), 'cache', 'workplace-discrimination-1-A1-simplified.txt');
