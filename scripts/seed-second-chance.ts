...
function getSecondChanceMetadata() {
  const genres: string[] = ['Modern Story', 'Inspirational', 'Second Chance'];
  const themes: string[] = ['Second Chance', 'Redemption', 'Finding Redemption', 'Persistence', 'Identity Transformation', 'Overcoming Stigma', 'Belonging', 'Building New Life'];
  const moods: string[] = ['Inspiring', 'Hopeful', 'Empowering', 'Uplifting', 'Resilient'];
  return { genres, themes, moods };
}

async function seedSecondChance() {
  console.log('🎤 Seeding "Second Chance: Finding Redemption" to Modern Voices Collection...\n');

  const storyId = 'second-chance-1';
  const metadata = getSecondChanceMetadata();

  // 212 sentences for A1 level, average 6.3 words/sentence, 80 WPM
  const sentences = 212;
  const bundles = 53; // 4 sentences per bundle
  const readingTime = Math.ceil((sentences * 6.9) / 80); // ~20 minutes

  const secondChanceStory = {
    id: storyId,
    title: 'Second Chance: Finding Redemption',
    author: 'BookBridge', // Original narrative based on extracted themes
    description: 'An inspiring story about Maria, who loses her hearing at age 18 and watches her dream of becoming a choir director die. Through deep depression, her father\'s encouragement, and finding new ways to make music, she transforms her life and achieves her dreams. This powerful tale shows how people adapt, persist, and overcome challenges. A1 level with Jane voice.',
    sentences: sentences,
    bundles: bundles,
  ...
  const book = await prisma.featuredBook.upsert({
    where: { slug: secondChanceStory.id },
    create: {
      slug: secondChanceStory.id,
      title: secondChanceStory.title,
      author: secondChanceStory.author,
      description: secondChanceStory.description,
      sentences: secondChanceStory.sentences,
      bundles: secondChanceStory.bundles,
      gradient: convertGradient(secondChanceStory.gradient),
      abbreviation: secondChanceStory.abbreviation,
  ...
  console.log('\n🎉 "Second Chance: Finding Redemption" seeded successfully!');
  console.log('\n💡 Next steps:');
  ...
  console.log('   5. Test reading "Second Chance: Finding Redemption"');
}
