 ...
 * (Same algorithm as bundle generation)
 */
 ...
 */
async function generateCombinedPreviewAudio(combinedText, bookId, level) {
  console.log(`\n🎵 Generating combined preview audio for "${bookId}" at ${level} level...`);
  ...
  try {
    ...
    const tempOriginalFile = path.join(tempDir, `${bookId}-${level}-preview-combined-original.mp3`);
    ...
    const tempSlowedFile = path.join(tempDir, `${bookId}-${level}-preview-combined-slowed.mp3`);
    ...
    // Split combined text into sentences for Enhanced Timing v3 calculation
    console.log(`   🔧 Calculating Enhanced Timing v3 sentence timings...`);
    const sentenceRegex = /([^.!?]+[.!?]+)/g;
    const sentences = combinedText.match(sentenceRegex) || [];
    const cleanedSentences = sentences.map(s => s.trim()).filter(s => s.length > 0);
    
    if (cleanedSentences.length === 0) {
      throw new Error('No sentences found in combined text');
    }

    // Calculate precise sentence timings using Enhanced Timing v3
    const sentenceTimings = calculateEnhancedTimingV3(cleanedSentences, measuredDuration);
    console.log(`   ✅ Calculated timings for ${sentenceTimings.length} sentences`);

    // Upload slowed audio to Supabase
    const audioFileName = `${bookId}/${level}/preview-combined.mp3`;
...
async function main() {
  console.log('🎤 Generating combined preview for Helen Keller\'s "The Story of My Life"');

  // Load combined text from cache
  const cacheDir = path.join(process.cwd(), 'cache');
  const combinedTextPath = path.join(cacheDir, `${BOOK_ID}-${CEFR_LEVEL}-preview-combined.txt`);
  
  if (!fs.existsSync(combinedTextPath)) {
    throw new Error(`Combined text file not found: ${combinedTextPath}`);
  }
  ...
  // Generate audio
  const audioMetadata = await generateCombinedPreviewAudio(combinedText, BOOK_ID, CEFR_LEVEL);

  // Save audio metadata to cache (including sentence timings)
  const audioMetadataPath = path.join(cacheDir, `${BOOK_ID}-${CEFR_LEVEL}-preview-combined-audio.json`);
  const metadata = {
    bookId: BOOK_ID,
    level: CEFR_LEVEL,
    audio: {
  ...
  console.log('\n🎉 Combined preview generation complete!');
