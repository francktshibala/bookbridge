...
async function integrateDatabaseRecords() {
  console.log('🗄️ PHASE 4: DATABASE INTEGRATION (A1)');
  ...
  try {
    ...
    const simplifiedTextPath = path.join(process.cwd(), 'cache/ted-talks/danger-of-single-story-a1-simplified.txt');
    ...
    for (const bundle of metadata) {
      try {
        ...
        // URL format: https://xsolwqqdbsuydwmmwtsl.supabase.co/storage/v1/object/public/audio-files/danger-of-single-story/A1/bundle_X.mp3
        const relativeAudioPath = bundle.audioUrl.includes('/audio-files/')
          ? bundle.audioUrl.split('/audio-files/')[1]
          : `danger-of-single-story/A1/bundle_${bundle.bundleIndex}.mp3`;
