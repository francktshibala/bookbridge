...
async function generateCombinedPreview() {
  console.log('📝 Generating combined preview text for Community Builder #2...');
  ...
  const prompt = `You are creating a combined preview text for an ESL story at A1 level about community building (wrong turn → soccer team → refugee school network).
...
2. PREVIEW (50-75 words): Meta-description style
   ...
   - Describe the journey: privileged beginning → exile → wrong turn → school founding
   - Main theme: community building, belonging, transformation, educational justice
   - Key insight: one person can change a neighborhood, mutual healing, soccer as universal language
   - Impact: message about hope, belonging, and building community

3. EMOTIONAL HOOK (50-100 words): Start with struggle
   - Use the existing hook as inspiration but simplify to A1 level
   - Keep the emotional impact: exile, drifting, wrong turn, discovery

4. BACKGROUND CONTEXT (30-50 words): Neutral, factual tone
   - Set the scene: refugees face educational challenges, language barriers
