...
- Keep proper nouns: Maya, Emma, Sofia, etc.
`;
...
6. Keep proper nouns unchanged (Maya, Emma, Sofia, etc.)
...
async function simplifyStory() {
  const inputFile = path.join(process.cwd(), 'cache', 'youth-activism-1-original.txt');
  const outputFile = path.join(process.cwd(), 'cache', 'youth-activism-1-A1-simplified.txt');
