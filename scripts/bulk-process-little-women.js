...
async function detectChunkCount() {
  ...
  const maxChunks = 500 // Maximum to check
  let detectedCount = 0
...
async function main() {
  ...
  console.log(`${colors.bright}${colors.magenta}📚 BULK PROCESSING LITTLE WOMEN${colors.reset}`)
  ...
  if (finalCheck.total >= chunkCount * CEFR_LEVELS.length) {
    console.log('')
    console.log(`${colors.green}${colors.bright}🎉 LITTLE WOMEN FULLY PROCESSED!${colors.reset}`)
    console.log(`${colors.green}📚 All CEFR levels ready for instant access!${colors.reset}`)
  }
