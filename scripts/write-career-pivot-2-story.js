...
async function writeMainStory() {
  console.log('✍️  Writing original Career Pivot #2 narrative story at A1 level...');
...
- Choose names that fit the story's cultural context (career pivot stories involve diverse backgrounds)
- Use unique names per story to avoid confusion
- NOT real names from sources (NOT Tracy Sharp, NOT Sabrina Ling, NOT Somya Lohia - use generic names)
...
- Structure: unexpected job loss/crisis → struggle → breakthrough → transformation/new path
...
- CRISIS: ${themes.storyArc.crisis.join(', ')}
...
1. Starts with the emotional hook energy (unexpected job loss, financial crisis, or career barrier)
2. Develops key emotional moments in sequence:
   - Initial shock/crisis (fired, laid off, credential barrier, financial crisis)
   - Deep struggle (identity loss, financial pressure, isolation, fear)
   - Breakthrough moments (therapeutic help, support from others, new opportunity, persistence)
   - Transformation (new career path, service-oriented work, finding purpose, hope restored)
3. Shows the emotional arc: unexpected loss/crisis → struggle → breakthrough → transformation/new path
...
5. Combines elements from all source themes:
   - Fired → therapeutic breakthrough → service career (Source 1)
   - Layoff → financial crisis → thriving consultancy (Source 2)
   - Layoff → multiple pivots → long-term transformation (Source 3)
   - Immigrant barrier → persistence → maintained hope (Source 4)
6. Ends with realization of new purpose, belonging, and hope
