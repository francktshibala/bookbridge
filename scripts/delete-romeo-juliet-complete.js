...
async function deleteRomeoJulietComplete() {
  try {
    console.log('🗑️  Complete Romeo and Juliet deletion starting...\n');

    // Define all Romeo and Juliet book IDs
    const bookIds = ['gutenberg-1513', 'gutenberg-1513-A1'];
    ...
    const cacheFiles = [
      './cache/romeo-juliet.json',
      './cache/romeo-juliet-original.json',
      './cache/romeo-juliet-modernized.json',
      './cache/romeo-juliet-a1-bundles.json'
    ];
    ...
    const dataDir = './data/romeo-juliet';
    ...
    console.log('\n' + '='.repeat(60));
    console.log('✅ Romeo and Juliet deletion complete!');
    ...
    const remainingBooks = await prisma.book.count({
      where: {
        OR: [
          { id: 'gutenberg-1513' },
          { id: 'gutenberg-1513-A1' }
        ]
      }
    });

    if (remainingChunks === 0 && remainingBooks === 0) {
      console.log('✅ All Romeo and Juliet data successfully removed');
    } else {
