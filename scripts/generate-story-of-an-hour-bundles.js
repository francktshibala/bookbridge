...
const BOOK_ID = 'story-of-an-hour';
...
if (!targetLevel) {
  ...
  console.log('Usage: node scripts/generate-story-of-an-hour-bundles.js A1 [--pilot]');
  process.exit(1);
}
...
async function checkElevenLabsQuota() {
  try {
    ...
    // Estimate cost (rough: ~1000 words = ~5,000 chars = ~$0.83)
    const estimatedCost = Math.ceil(1000 / 1000) * 16.5; // cents
    console.log(`   Estimated cost for ~1000 words: ~$${(estimatedCost / 100).toFixed(2)}`);
...
async function generateStoryOfAnHourBundles() {
  console.log(`\n📚 Starting bundle generation for "The Story of an Hour" (${CEFR_LEVEL})...`);
  ...
  try {
    ...
    console.log(`\n🎉 The Story of an Hour bundle generation completed!`);

  } catch (error) {
    console.error(`\n❌ Error generating bundles:`, error.message);
    throw error;
  } finally {
...
export default generateStoryOfAnHourBundles;
