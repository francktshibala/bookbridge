/**
 * Generate combined preview text (Preview + Background + Hook) for Disability Overcome #2 story
...
async function generateCombinedPreview() {
  console.log('📝 Generating combined preview text for Disability Overcome #2...');
...
- Describe Lucas and his challenge (going blind, mountaineering)
- Mention the main theme (disability overcome, adaptation, finding new ways, mountaineering)
- Key insight: "Through [method], [he] achieved..."
- Impact: "A [adjective] message about [outcome]..."
- Use character name "Lucas" (NOT Erik Weihenmayer or real names)
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are creating combined preview text for A1 level ESL learners. You follow the exact format with double newlines between sections. Your writing is simple, clear, and appropriate for A1 level (short sentences, simple words, basic grammar). You use character name "Lucas", never real names from sources.',
        },
        {
          role: 'user',
          content: prompt,
