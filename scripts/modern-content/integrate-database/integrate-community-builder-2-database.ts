/**
 * Integrate Community Builder #2 story into database
...
const BOOK_ID = 'community-builder-2';
const CEFR_LEVEL = 'A1';
...
async function integrateDatabase() {
  console.log(`📚 INTEGRATING "Community Builder #2 Story" - A1 LEVEL WITH ENHANCED TIMING V3`);
  console.log(`=`.repeat(60));

  try {
    // Load story text (use simplified version)
    const storyTextPath = path.join(process.cwd(), 'cache', `${BOOK_ID}-A1-simplified.txt`);
    if (!fs.existsSync(storyTextPath)) {
      throw new Error(`Story file not found: ${storyTextPath}`);
    }
    const storyText = fs.readFileSync(storyTextPath, 'utf-8');
