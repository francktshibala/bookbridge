...
// VALIDATED VOICE ID - Daniel (Business/professional narrator)
...
// PRODUCTION VOICE SETTINGS - Daniel (Disability Overcome narration)
...
async function measureAudioDuration(audioFile) {
  try {
    ...
    console.error(`   ❌ Failed to measure duration: ${error.message}`);
    return null;
  }
}

async function generateBundles() {
  const storyFile = path.join(process.cwd(), `cache/${STORY_ID}-${CEFR_LEVEL}-original.txt`);
  
  if (!fs.existsSync(storyFile)) {
    throw new Error(`Story file not found: ${storyFile}. Run write-disability-overcome-2-story.js first.`);
  }

  const storyText = fs.readFileSync(storyFile, 'utf-8');
  // Remove hook if it's at the beginning (it's already in preview-combined)
  const storyWithoutHook = storyText.replace(/^Imagine you are.*?tallest mountain\.\s*/s, '');
  const sentences = storyWithoutHook
    .split(/(?<=[.!?])\s+/)
    .map(s => s.trim())
    .filter(s => s.length > 0);

  console.log(`\n📖 Loaded ${sentences.length} sentences from A1 story`);
