...
// VALIDATED VOICE ID - Daniel (British deep news presenter)
...
async function generateBundles() {
  const storyFile = path.join(process.cwd(), `cache/${STORY_ID}-${CEFR_LEVEL}-simplified.txt`);
  
  if (!fs.existsSync(storyFile)) {
    throw new Error(`Story file not found: ${storyFile}. Run simplify-community-builder-2.js first.`);
  }
