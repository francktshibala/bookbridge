/**
 * Expand the A1 Refugee Journey #2 story to meet target length (15-20 minutes A1 level)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 Refugee Journey #2 story to meet target length...');

  // Load current story
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-refugee-journey-2-story.js first.');
    return;
  }
  ...
  const prompt = `You are expanding a Refugee Journey story #2 for ESL learners at A1 level. The current story is ${currentWordCount} words, and you need to expand it to approximately ${targetWordCount} words (target: 20 minutes reading time).
...
- Keep generic character names (Amina, Rami, Solomon, Helen - already used in story)
...
- Add more detail to emotional moments (especially high-weight moments):
  ...
  * Long wait/perseverance (years apart, video calls, legal battles, survival in camp)
...
- Include more moments showing the struggle and perseverance (12 years of waiting, video calls, legal battles)
- Add more dialogue and interactions between characters
- Expand the breakthrough moments with more detail (reunion scene, becoming a teacher, helping others)
