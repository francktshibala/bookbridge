/**
 * Expand the A1 Community Builder #2 story to meet target length (20 minutes A1 level)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 Community Builder #2 story to meet target length...');

  // Load current story
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-community-builder-2-story.js first.');
    return;
  }
  ...
  const prompt = `You are expanding a Community Builder story #2 for ESL learners at A1 level. The current story is ${currentWordCount} words, and you need to expand it to approximately ${targetWordCount} words (target: 20 minutes reading time).
...
- Keep generic character names (Layla - already used in story)
...
- Include more moments showing the struggle and perseverance (years of drifting, building the school)
- Add more dialogue and interactions between characters
- Expand the breakthrough moments with more detail (wrong turn scene, school founding, graduation)
...
Expand the story by adding detail, emotion, dialogue, and scenes while keeping all existing content. Maintain A1 level language (short sentences, simple words). Return ONLY the expanded story text, no explanations.`;
  ...
  try {
    ...
    // Save expanded story (overwrite original)
