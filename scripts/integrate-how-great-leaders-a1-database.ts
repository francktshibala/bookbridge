...
async function integrateDatabaseRecords() {
  console.log('🗄️ PHASE 5: DATABASE INTEGRATION (A1)');
  ...
  try {
    ...
    const simplifiedTextPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-a1-simplified.txt');
    const previewTextPath = path.join(process.cwd(), 'cache/how-great-leaders-inspire-action-A1-preview.txt');
    ...
    for (const bundle of metadata) {
      try {
        ...
        // URL format: https://xsolwqqdbsuydwmmwtsl.supabase.co/storage/v1/object/public/audio-files/how-great-leaders-inspire-action/A1/bundle_X.mp3
        const relativeAudioPath = bundle.audioUrl.includes('/audio-files/')
          ? bundle.audioUrl.split('/audio-files/')[1]
          : `how-great-leaders-inspire-action/A1/bundle_${bundle.bundleIndex}.mp3`;
    ...
    console.log(`=`.repeat(60));
    console.log(`📚 Book: How Great Leaders Inspire Action by Simon Sinek`);
