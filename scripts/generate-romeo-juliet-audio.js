...
// Configuration
const BOOK_ID = 'gutenberg-1513-A1';
const CEFR_LEVEL = 'A1';
...
async function generateRomeoJulietAudio() {
  try {
    console.log(`🎵 Generating Romeo and Juliet audio ${isPilot ? '(PILOT - 5 bundles)' : '(FULL)'} with Sarah voice...\n`);
    ...
    if (chunks.length === 0) {
      console.log('✅ All bundles already have audio generated');
      return;
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      console.log(`\n🔄 Generating audio for bundle ${chunk.chunkIndex + 1}/${isPilot ? 5 : 'total'}...`);

      try {
        // CRITICAL: Ensure exactly 4 sentences per bundle
...
export { generateRomeoJulietAudio };