...
// VALIDATED VOICE ID - Daniel (Professional/inspirational narrator)
...
// PRODUCTION VOICE SETTINGS - Daniel (Single Parent Rising narration)
...
if (!VALID_LEVELS.includes(targetLevel)) {
  ...
  console.log('Usage: node scripts/generate-single-parent-rising-1-preview-audio.js [A1|A2]');
  process.exit(1);
}
...
async function main() {
  console.log('🎤 Generating combined preview audio for "Single Parent Rising #1"');
  ...
  if (!fs.existsSync(combinedTextPath)) {
    throw new Error(`Combined text file not found: ${combinedTextPath}. Run generate-single-parent-rising-1-preview-combined.js first.`);
  }
