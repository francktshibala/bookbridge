/**
 * Expand the A1 story to meet target length (15-20 minutes A1 level)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 story to meet target length...');
  
  // Load current story
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-refugee-journey-story.js first.');
    return;
  }
...
- Keep generic character names (Amina, Samir, Layla, etc.)
- Target: ${targetWordCount} words total (approximately 20 minutes A1 reading time)

EXPANSION FOCUS:
- Add more detail to emotional moments (especially very high and high-weight moments)
...
- Expand the epiphany moment (sandstorm) with more detail
- Add more scenes showing transformation and giving back
- Deepen the emotional journey

KEY EMOTIONAL MOMENTS TO EXPAND:
${themes.emotionalMoments
  .filter(m => m.emotionalWeight === 'very high' || m.emotionalWeight === 'high')
  .map((m, i) => `${i + 1}. ${m.moment} (${m.emotionalWeight} weight) - expand this moment`)
  .join('\n')}

CURRENT STORY:
${currentStory}

Expand the story by adding detail, emotion, and scenes while keeping all existing content. Maintain A1 level language (short sentences, simple words). Return ONLY the expanded story text, no explanations.`;
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a skilled storyteller expanding narratives for A1 level ESL learners. You preserve all existing content while adding detail, emotion, and scenes to reach the target length. Your writing is simple, clear, and appropriate for A1 level learners (short sentences, simple words, basic grammar).',
        },
        {
          role: 'user',
          content: prompt,
