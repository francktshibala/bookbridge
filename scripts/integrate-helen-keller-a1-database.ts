...
console.log(`📚 INTEGRATING "Helen Keller - The Story of My Life" - A1 LEVEL WITH ENHANCED TIMING V3`);
...
async function integrateDatabase() {
  console.log(`\n📖 Step 1: Loading original text (modernized)...`);
  const originalTextPath = path.join(process.cwd(), 'cache', `${BOOK_ID}-modernized.txt`);
  ...
  console.log(`🔧 Timing: Enhanced Timing v3 (character-count + punctuation penalties)`);
  console.log(`\n💡 Next steps:`);
  console.log(`   1. Create API endpoint: app/api/${BOOK_ID}-${CEFR_LEVEL.toLowerCase()}/bundles/route.ts`);
  console.log(`   2. Update frontend config: lib/config/books.ts`);
  console.log(`   3. Test reading route: /read/${BOOK_ID}`);
}

integrateDatabase()
  .catch((error) => {
    console.error('❌ Error integrating database:', error);
    process.exit(1);
  })
  .finally(async () => {
