...
if (!VALID_LEVELS.includes(targetLevel)) {
  ...
  console.log('Usage: node scripts/generate-single-parent-rising-1-preview-combined.js [A1|A2]');
  process.exit(1);
}
...
async function generateCombinedPreview() {
  ...
  if (!fs.existsSync(storyFile)) {
    ...
    console.log(`   Please run simplification first: node scripts/simplify-single-parent-rising-1.js ${CEFR_LEVEL}`);
    process.exit(1);
  }
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are an expert at creating preview texts for ${CEFR_LEVEL} level ESL stories. You write at the appropriate complexity level, use appropriate vocabulary, and follow exact formatting requirements.`,
        },
        {
          role: 'user',
          content: prompt,
