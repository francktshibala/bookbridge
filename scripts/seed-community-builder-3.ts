...
function getCommunityBuilder3Metadata() {
  const genres: string[] = ['Modern Story', 'Inspirational', 'Community Builder'];
  const themes: string[] = ['Community Building', 'Belonging', 'Transformation', 'Loneliness', 'Taking Action', 'Storytelling', 'Connection', 'Creating Community'];
  const moods: string[] = ['Inspiring', 'Hopeful', 'Empowering', 'Uplifting', 'Resilient', 'Emotional'];
  return { genres, themes, moods };
}

async function seedCommunityBuilder3() {
  console.log('🎤 Seeding "Creating Belonging Through Action" to Modern Voices Collection...\n');

  const storyId = 'community-builder-3';
  const metadata = getCommunityBuilder3Metadata();

  // 213 sentences for A1 level, average 7.5 words/sentence, 80 WPM
  const sentences = 213;
  const bundles = 43; // 5 sentences per bundle
  const readingTime = Math.ceil((sentences * 7.5) / 80); // ~20 minutes

  const communityBuilder3Story = {
    id: storyId,
  ...
  const book = await prisma.featuredBook.upsert({
    where: { slug: communityBuilder3Story.id },
    create: {
      slug: communityBuilder3Story.id,
      title: communityBuilder3Story.title,
      author: communityBuilder3Story.author,
      description: communityBuilder3Story.description,
      sentences: communityBuilder3Story.sentences,
      bundles: communityBuilder3Story.bundles,
      gradient: convertGradient(communityBuilder3Story.gradient),
      abbreviation: communityBuilder3Story.abbreviation,
  ...
  console.log('\n🎉 "Creating Belonging Through Action" seeded successfully!');
  console.log('\n💡 Next steps:');
  ...
  console.log('   5. Test reading "Creating Belonging Through Action"');
}

seedCommunityBuilder3()
  .catch(error => {
    console.error(`\n💥 Fatal error:`, error);
    process.exit(1);
  })
