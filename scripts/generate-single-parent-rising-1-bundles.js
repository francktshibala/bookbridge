...
if (!VALID_LEVELS.includes(targetLevel)) {
  ...
  console.log('Usage: node scripts/generate-single-parent-rising-1-bundles.js [A1|A2] [--pilot]');
  process.exit(1);
}
...
async function generateBundles() {
  try {
    ...
    for (let i = 0; i < bundlesToGenerate; i++) {
      ...
      const audioBuffer = await generateElevenLabsAudio(bundleText, DANIEL_VOICE_SETTINGS);
