/**
 * Integrate Second Chance story into database
...
async function integrateDatabase() {
  console.log(`📚 INTEGRATING "Second Chance Story" - A1 LEVEL WITH ENHANCED TIMING V3`);
  ...
  try {
    ...
    console.log(`📝 Sentences: ${originalText.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 0).length}`);
    ...
    const bookContent = await prisma.bookContent.upsert({
      ...
      create: {
        bookId: BOOK_ID,
        title: 'Second Chance: Finding New Ways',
        author: 'BookBridge',
