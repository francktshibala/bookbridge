/**
 * Expand Disability Overcome story to meet target length (1,600-1,700 words, ~20 minutes)
 */
...
async function expandStory() {
  console.log('📖 Expanding A1 Disability Overcome story to meet target length...');
  
  const storyFile = path.join(CACHE_DIR, `${STORY_ID}-A1-original.txt`);
  if (!fs.existsSync(storyFile)) {
    console.error('❌ Story file not found. Run write-disability-overcome-story.js first.');
    return;
  }
...
2. Adding more scenes showing the struggle, depression, and isolation
3. Expanding the breakthrough moment with more detail
4. Adding more scenes showing persistence and practice
5. Adding more scenes showing transformation and success
6. Including more emotional moments and internal thoughts
7. Adding more dialogue between characters
8. Showing more of the adaptation process (how she learned new ways)
