/**
 * Integrate Workplace Discrimination #1 story into database
...
const BOOK_ID = 'workplace-discrimination-1';
const CEFR_LEVEL = 'A1';
...
async function integrateDatabase() {
  console.log(`📚 INTEGRATING "Workplace Discrimination #1" - A1 LEVEL WITH ENHANCED TIMING V3`);
  ...
  try {
    ...
    const metadataPath = path.join(process.cwd(), 'cache', `${BOOK_ID}-A1-bundles-metadata.json`);
