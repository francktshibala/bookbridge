...
console.log(`📚 INTEGRATING "How Great Leaders Inspire Action" - A2 LEVEL INTO DATABASE`);
...
async function integrateDatabase() {
  ...
  const simplifiedTextPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-a2-simplified.txt');
  ...
  const metadataPath = path.join(process.cwd(), 'cache/ted-talks/how-great-leaders-inspire-action-A2-bundles-metadata.json');
  ...
  console.log(`\n🔨 Step 5: Deleting existing A2 chunks (if any)...`);
  ...
  for (const bundle of metadata) {
    try {
      ...
      const relativeAudioPath = bundle.audioUrl.includes('/audio-files/')
        ...
        : `how-great-leaders-inspire-action/A2/bundle_${bundle.bundleIndex}.mp3`;
