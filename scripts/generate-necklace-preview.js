...
const BOOK_ID = process.argv[2] || 'the-necklace';
...
async function generatePreview() {
  ...
  try {
    // Load simplified A1 text to understand content
    ...
    console.log(`📊 Book stats: ${sentences.length} sentences, ${wordCount} words, ~${readingTimeMinutes} min read`);

    // Get first few sentences for context (avoid spoilers)
    const previewContext = sentences.slice(0, 5).join(' ');

    // Generate preview using OpenAI
    const prompt = `You are an ESL reading specialist creating a book preview for ESL students.

BOOK: "The Necklace" by Guy de Maupassant
