...
### **PARTNERSHIP_AND_BUSINESS_MODEL_STRATEGY.md**
**Location**: `/docs/PARTNERSHIP_AND_BUSINESS_MODEL_STRATEGY.md`
**Description**: Comprehensive business model analysis and partnership strategy document for BookBridge. **BUSINESS MODEL**: Detailed analysis of For-Profit, Non-Profit, and Hybrid (B-Corp) models with pros/cons, financial projections, and recommendation for B-Corp structure. **PARTNERSHIPS**: Complete partnership pitches for TED Talks and The Moth, including contact information, talking points, and value propositions. **MONETIZATION**: Paid features for ESL programs (student management, analytics, assignments, branding), pricing strategy, and revenue projections ($240K Year 1 → $4.1M Year 3). **FOUNDER CONCERNS**: Addresses concerns about program payment, competition, free user conversion, team attraction, and business model uncertainty. **RISK MITIGATION**: Strategies for addressing each concern with realistic timelines and action plans. **KEY RECOMMENDATION**: B-Corp model balances mission alignment with financial success, enabling path to millionaire status while maintaining "free for everyone" vision. Essential reference for strategic business decisions and partnership negotiations.

### **TEXTBOOK_STORIES_PUBLIC_DOMAIN_ANALYSIS.md**
**Location**: `/docs/TEXTBOOK_STORIES_PUBLIC_DOMAIN_ANALYSIS.md`
**Description**: Comprehensive analysis of stories from textbook table of contents to identify public domain works suitable for BookBridge. **ANALYSIS**: Evaluates 50+ stories from textbook units, categorizing them as Public Domain (safe to use), Borderline (needs verification), or Copyrighted (cannot use). **PUBLIC DOMAIN STORIES**: Identifies 15+ classic stories including "The Interlopers" (Saki), "The Invalid's Story" (Mark Twain), "The Red-headed League" (Conan Doyle), "Casey at the Bat" (Thayer), and works by Poe, Maupassant, Shakespeare, Dickinson, and others. **COPYRIGHTED STORIES**: Documents modern authors (Isaac Bashevis Singer, Julia Alvarez, Maya Angelou, Amy Tan, etc.) that cannot be used without permission. **RECOMMENDATIONS**: Prioritizes "The Interlopers" and "The Red-headed League" as best options for immediate implementation due to high friend-recommendation potential, engaging plots, and appropriate length (10-25 minutes). **ALREADY IN LIBRARY**: Notes that "The Gift of the Magi" (O. Henry) and "The Necklace" (Maupassant) are already implemented. Essential reference for content curation and avoiding copyright issues.

### **DICTIONARY_SCROLL_FIX_IMPLEMENTATION.md**
**Location**: `/docs/DICTIONARY_SCROLL_FIX_IMPLEMENTATION.md`
**Description**: Complete implementation guide for fixing dictionary accidental opens during scrolling. **PROBLEM**: Dictionary was opening accidentally when users scrolled up/down because scroll gestures triggered word clicks. **SOLUTION**: Combined approach using scroll detection (300ms cooldown) and increased thresholds (drag: 5px→10px, click timeout: 200ms→300ms). **IMPLEMENTATION**: Added scroll event listeners (wheel, touchmove, scroll), scroll state tracking with cooldown timer, and blocking logic in mouse/touch handlers. **TESTING**: Comprehensive checklist for desktop and mobile testing scenarios. **STATUS**: ✅ Implemented and tested - Dictionary now blocked during active scrolling and 300ms after scroll ends. **FILES MODIFIED**: `hooks/useDictionaryInteraction.ts` - Added scroll detection, increased thresholds, and cooldown logic. Essential reference for understanding dictionary interaction improvements and preventing accidental opens.

### **DICTIONARY_SCROLL_FIX_SUGGESTIONS.md**
**Location**: `/docs/DICTIONARY_SCROLL_FIX_SUGGESTIONS.md`
**Description**: Analysis document presenting 5 solution options for fixing dictionary scroll issues. **OPTIONS**: Scroll Detection + Cooldown (recommended), Increased Thresholds, Double-Click, Dictionary Button Mode, and Velocity-Based Detection. **COMPARISON**: Detailed pros/cons matrix with difficulty ratings, effectiveness scores, and UX impact analysis. **RECOMMENDATION**: Option 1 (Scroll Detection) combined with Option 2 (Increased Thresholds) for maximum protection. **IMPLEMENTATION PRIORITY**: Quick fix (increase thresholds) → Proper fix (scroll detection) → Future enhancement (velocity detection if needed). Provides decision framework for choosing the best solution approach.
...
**Description**: **COMPREHENSIVE AUTHENTICATION RELIABILITY PLAN** - Complete implementation plan for reliable authentication system with full conversion tracking. **CURRENT STATUS**: Email confirmation working ✅, password saving fixed ✅ (Phase 1 complete), password reset partially complete ⚠️ (redirect issue blocking - deferred), email verification tracked ✅ (Phase 3 complete). **GOALS**: (1) ✅ Fix password saving when Supabase email fails, (2) ⚠️ Add password reset functionality (deferred - redirect issue), (3) ✅ Track email verification in PostHog, (4) ⏳ Improve error handling (documented with 5 detailed steps), (5) ⏳ Complete conversion funnel tracking (documented with 4 detailed steps). **POSTHOG INTEGRATION**: Maps auth events to 4 conversion gates (Signup → First Use → Wow → Retention) as defined in `POSTHOG_ANALYTICS_IMPLEMENTATION_PLAN.md`. **IMPLEMENTATION PHASES**: 5 phases (6-10 days total) - Phase 1: Fix password saving ✅ COMPLETE, Phase 2: Add password reset ⚠️ PARTIALLY COMPLETE (deferred), Phase 3: Track email verification ✅ COMPLETE, Phase 4: Improve error handling ⏳ DOCUMENTED (5 detailed steps with code patterns), Phase 5: Complete funnel tracking ⏳ DOCUMENTED (4 detailed steps with code patterns). **DETAILED IMPLEMENTATION STEPS**: Both Phase 4 and Phase 5 include step-by-step guides with code patterns, file locations, and implementation details ready for execution. **CROSS-REFERENCES**: Links to `CONFIRMATION_EMAIL_SOLUTION.md`, `CONFIRMATION_EMAIL_FINAL_PLAN.md`, `POSTHOG_ANALYTICS_IMPLEMENTATION_PLAN.md`, `Agent1_Email_Service_Findings.md`, `Agent2_Supabase_Auth_Findings.md`, `SPF_VERIFICATION_INVESTIGATION.md`. **SUCCESS METRICS**: Password save success ≥99%, reset success ≥95%, login success ≥98%, signup → email verified ≥85%, overall signup → D7 retention ≥19%. Essential reference for authentication reliability improvements and conversion tracking implementation.
...
### **TEACHER_DASHBOARD_V2_IMPLEMENTATION_PLAN.md** 🔨 **NOT STARTED — BLOCKED ON DATA PIPELINE BUG**
**Location**: `/docs/implementation/TEACHER_DASHBOARD_V2_IMPLEMENTATION_PLAN.md`
**Description**: Synthesizes 3-agent research (`docs/research/Agent1/2/3_Teacher_Dashboard_*_Findings.md`) into a build plan for Teacher Dashboard v2. **Critical blocker (Phase 0)**: confirmed directly against production that both `reading_sessions` (0 rows, no writer) AND `reading_positions` (0 rows, despite a fully-wired write path) are empty — v1's "books read"/"last active" numbers are showing 0/blank for every real student today, and simply repointing the query (the research agents' top fix) won't help since the target table is also empty. Only `user_scores` (quiz system) has real data. Needs investigation (client invocation, server logs, RLS rejection — leading hypothesis, unconfirmed) before any v2 UI work. Once unblocked: consolidate the roster query into one parameterized statement (no caching/job infra needed at current scale — infrastructure audit found none exists), surface per-book progress + latest quiz score (never a per-student average — quiz format makes averages misleading) + CEFR level per student, and lead the UI with a "needs your attention" action list (named students + one action each) rather than a stats grid, per convergent competitor research (Google Classroom, Lexia, Microsoft Reading Progress). Explicitly deferred: time-spent-reading (no instrumentation exists, every shortcut is a trap), comprehension skill-type breakdown (no per-question response table exists), class-level aggregates, school/org-level views. Essential reference before writing any Teacher Dashboard v2 code.

### **ROLE_BASED_SIGNUP_COMPLETION_SUMMARY.md** ✅ **COMPLETE — LIVE IN PRODUCTION**
**Location**: `/docs/implementation/ROLE_BASED_SIGNUP_COMPLETION_SUMMARY.md`
**Description**: Final verification results for role-based signup. `NEXT_PUBLIC_ROLE_BASED_SIGNUP=true` in production, manually verified end-to-end (signup, role persisted, email confirmed, login, role-based redirect, catalog renders correctly) for both Student and Teacher. Documents a real bug found and fixed via browser testing (Prisma `users` row wasn't created on the common signup path — only in a rarely-hit branch), plus three known non-blocking issues logged for later: confirmation magic links don't auto-sign-in (implicit-flow hash tokens vs. the callback route's PKCE-only handling), a testing gotcha where re-signing-up with an existing email silently updates it, and minor Supabase/Prisma role-metadata drift on that same path. Essential reference before touching the confirmation-email flow or the `create-user` route again.

### **ROLE_BASED_SIGNUP_IMPLEMENTATION_PLAN.md** ✅ **COMPLETE**
**Location**: `/docs/implementation/ROLE_BASED_SIGNUP_IMPLEMENTATION_PLAN.md`
**Description**: The how, companion to `ROLE_BASED_SIGNUP_DESIGN_RATIONALE.md` (the why). Five phases, all complete and merged: (0) additive nullable `role` column on Prisma `User`, no behavior change, (1) TDD pure role-decision logic modeled on the existing `password-reset-intent.ts` test pattern, (2) persist role through the resilient `create-user` admin-API route rather than the fragile client path, fixing the placeholder-email lazy-upsert bug as part of the same change, (3) handle Google OAuth's missing role via a post-callback `/auth/select-role` prompt, (4) role-based post-login redirect sourced from `SimpleAuthProvider` only (`AuthProvider.tsx` explicitly untouched — user decision), (5) staging verification + feature flag, merged to `main`, deployed. See `ROLE_BASED_SIGNUP_COMPLETION_SUMMARY.md` for production verification results. Also documents feature order after this: Teacher Dashboard next, then one-click assignment, then shared reading lists; content pipeline/question-selector/vocab list are parallelizable; CEFR bug fix remains outstanding and independent.
