...
SCRIPT_FILE = os.path.join(os.path.dirname(__file__), "methods-video-script-section-3.txt")
OUTPUT_DIR = os.path.expanduser("~/Desktop/methods-video-section3-temp")
FINAL_OUTPUT = os.path.expanduser("~/Desktop/methods-video-section3.mp3")

# Create temp directory
Path(OUTPUT_DIR).mkdir(exist_ok=True)

print("🎙️ Generating audio for Section 3: What Speaking Really Means...")
...
for line in lines:
    ...
    if line and line != lines[-1].strip():
        ...
        if '?' in line or '...' in line or line.startswith('It\'s') or 'But here\'s the truth' in line:
            segments.append({"type": "pause", "duration": 2.0})
        elif line.endswith('you.') or 'That\'s what' in line or 'That\'s our goal' in line:
            segments.append({"type": "pause", "duration": 2.0})
        else:
...
print(f"\n✅ SUCCESS! Section 3 audio saved to: {FINAL_OUTPUT}")
