...
SCRIPT_FILE = os.path.join(os.path.dirname(__file__), "methods-video-script-section-1.txt")
OUTPUT_DIR = os.path.expanduser("~/Desktop/methods-video-section1-temp")
FINAL_OUTPUT = os.path.expanduser("~/Desktop/methods-video-section1.mp3")

# Create temp directory
Path(OUTPUT_DIR).mkdir(exist_ok=True)

print("🎙️ Generating audio for Section 1: The Hook...")
...
for line in lines:
    ...
    if line and line != lines[-1].strip():
        # Longer pause after questions or dramatic statements
        if '?' in line or '...' in line or line.endswith('wrong.'):
...
print(f"\n✅ SUCCESS! Section 1 audio saved to: {FINAL_OUTPUT}")
