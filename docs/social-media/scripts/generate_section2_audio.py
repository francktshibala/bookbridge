...
SCRIPT_FILE = os.path.join(os.path.dirname(__file__), "methods-video-script-section-2.txt")
OUTPUT_DIR = os.path.expanduser("~/Desktop/methods-video-section2-temp")
FINAL_OUTPUT = os.path.expanduser("~/Desktop/methods-video-section2.mp3")

# Create temp directory
Path(OUTPUT_DIR).mkdir(exist_ok=True)

print("🎙️ Generating audio for Section 2: The Biggest Lies...")
...
for line in lines:
    ...
    if line and line != lines[-1].strip():
        # Longer pause after questions, dramatic statements, or section headers
        if '?' in line or '...' in line or line.startswith('Lie Number') or 'completely wrong' in line.lower() or 'also wrong' in line.lower() or 'most dangerous' in line.lower():
            segments.append({"type": "pause", "duration": 2.0})
        elif line.endswith('wrong.') or 'You know what they found?' in line:
            segments.append({"type": "pause", "duration": 2.5})
        else:
...
print(f"\n✅ SUCCESS! Section 2 audio saved to: {FINAL_OUTPUT}")
