...
SCRIPT_FILE = os.path.join(os.path.dirname(__file__), "methods-video-script-section-6.txt")
OUTPUT_DIR = os.path.expanduser("~/Desktop/methods-video-section6-temp")
FINAL_OUTPUT = os.path.expanduser("~/Desktop/methods-video-section6.mp3")

# Create temp directory
Path(OUTPUT_DIR).mkdir(exist_ok=True)

print("🎙️ Generating audio for Section 6: CTA & Closing...")
...
for line in lines:
    ...
    if line and line != lines[-1].strip():
        # Longer pause after section headers, CTAs, or key points
        if line.startswith('First,') or line.startswith('Second,') or line.startswith('Third,') or 'Remember:' in line:
            segments.append({"type": "pause", "duration": 2.0})
        elif 'like this video' in line.lower() or 'subscribe' in line.lower() or 'download' in line.lower():
            segments.append({"type": "pause", "duration": 2.0})
        elif line.endswith('you.') or 'I believe' in line:
            segments.append({"type": "pause", "duration": 2.0})
        else:
...
print(f"\n✅ SUCCESS! Section 6 audio saved to: {FINAL_OUTPUT}")
