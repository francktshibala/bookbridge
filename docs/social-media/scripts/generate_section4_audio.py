...
SCRIPT_FILE = os.path.join(os.path.dirname(__file__), "methods-video-script-section-4.txt")
OUTPUT_DIR = os.path.expanduser("~/Desktop/methods-video-section4-temp")
FINAL_OUTPUT = os.path.expanduser("~/Desktop/methods-video-section4.mp3")

# Create temp directory
Path(OUTPUT_DIR).mkdir(exist_ok=True)

print("🎙️ Generating audio for Section 4: The Three Proven Methods...")
...
for line in lines:
    ...
    if line and line != lines[-1].strip():
        # Longer pause after questions, dramatic statements, method headers, or key points
        if '?' in line or '...' in line or line.startswith('METHOD') or 'How to Do It' in line or 'Why does this work' in line:
            segments.append({"type": "pause", "duration": 2.0})
        elif line.endswith('better.') or 'Here\'s Where' in line or 'Our Social Media' in line:
            segments.append({"type": "pause", "duration": 2.0})
        elif line.endswith('right now.') or line.endswith('every week.'):
            segments.append({"type": "pause", "duration": 2.0})
        else:
...
print(f"\n✅ SUCCESS! Section 4 audio saved to: {FINAL_OUTPUT}")
